...
physeq_filtered
sample_data(physeq_filtered)[["Group"]] <- factor(sample_data(physeq_filtered)[["Group"]], levels=c("SHAM_STD","SHAM_EE","TBI_STD","TBI_EE_COH"))
#Normalizar datos
...
```{r message=FALSE, warning=FALSE}
alfa_div <- estimate_richness(datos_sin_euc)
```
...
compare_means(Simpson~Group,alfa_div)
...
```{r message=FALSE, warning=FALSE}
datos_STD <- subset_samples(percentages, Group=="SHAM_STD" | Group=="TBI_STD") %>% filter_taxa(function(x) sum(x) > 0, T)
```
...
```{r message=FALSE, warning=FALSE}
###UNWEIGHTED
adonis2(UUdist ~ housing, data=data.frame(sample_data(percentages)))
###WEIGHTED:
adonis2(WUdist ~ housing, data=data.frame(sample_data(percentages)))
```
...
```{r message=FALSE, warning=FALSE}
adonis2(UUdist ~ injury/housing, data=data.frame(sample_data(percentages)),perm=9999)
###WEIGHTED:
adonis2(WUdist ~ injury/housing, data=data.frame(sample_data(percentages)),perm=9999)
```
...
```{r message=FALSE, warning=FALSE}
###WEIGHTED:
pwise.adonis.physeq(percentages, "Group", dist.m="wunifrac", 999, "BH")
###UNWEIGHTED:
pwise.adonis.physeq(percentages, "Group", dist.m="uunifrac", 999, "BH")
###BRAY-CURTIS DISTANCE:
pwise.adonis.physeq(percentages, "Group", dist.m="bray", 999, "BH")
```
...
genus_abundance <- plot_bar(glom_others(percentages,"Genus"), fill="Genus")+scale_fill_manual(values = genus_colors_abs)+facet_grid(.~Group,scales="free", space="free")+theme_light(base_size = 12, base_family = "", base_line_size = 1)+theme(axis.text.x = element_text(angle = 90))

genus_abundance

ggsave("genus_abundance.svg",plot = genus_abundance,dpi = 600, scale =1.5 )
```
